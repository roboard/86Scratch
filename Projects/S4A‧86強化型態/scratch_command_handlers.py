...
class ScratchCommandHandlers:
    ...
    ln_CLOCKWISE = config.get('translation_lists', 'ln_CLOCKWISE').split(',')
    ln_ANTICLOCKWISE = config.get('translation_lists', 'ln_ANTICLOCKWISE').split(',')
    ...
    #s4a
    def check_CMD_SERVO_DIR(self, command):
        if command in self.ln_CLOCKWISE:
            return 'clockwise'
        if command in self.ln_ANTICLOCKWISE:
    ...
    def __init__(self, firmata, com_port, total_pins_discovered, number_of_analog_pins_discovered):
        ...
        for pin in range(16):
            self.s4a_reporter_base.append(0)
        
        self.s4a_reporter_base[0] = 'A0 VALUE'
        self.s4a_reporter_base[1] = 'A1 VALUE'
        self.s4a_reporter_base[2] = 'A2 VALUE'
        self.s4a_reporter_base[3] = 'A3 VALUE'
        self.s4a_reporter_base[4] = 'A4 VALUE'
        self.s4a_reporter_base[5] = 'A5 VALUE'
        self.s4a_reporter_base[6] = 'D2 VALUE'
        self.s4a_reporter_base[7] = 'D3 VALUE'
        
        self.s4a_reporter_base[8] = 'getvalue/A0 VALUE'
        self.s4a_reporter_base[9] = 'getvalue/A1 VALUE'
        self.s4a_reporter_base[10] = 'getvalue/A2 VALUE'
        self.s4a_reporter_base[11] = 'getvalue/A3 VALUE'
        self.s4a_reporter_base[12] = 'getvalue/A4 VALUE'
        self.s4a_reporter_base[13] = 'getvalue/A5 VALUE'
        self.s4a_reporter_base[14] = 'getbool/D2 VALUE'
        self.s4a_reporter_base[15] = 'getbool/D3 VALUE'
    ...
    def poll(self, command):
        ...
        s4a_response_table = self.firmata.get_s4a_response_table()
        ...
        for pin in range(8):
            pin_number = str(pin)
            pin_entry = s4a_response_table[pin]
            if pin == 6 or pin == 7:
                if pin_entry == 0:
                    value = 'false'
                else:
                ...
                value = str(pin_entry)
            report_entry = self.s4a_reporter_base[pin]
            report_entry = report_entry.replace("VALUE", value)
            responses += report_entry
            responses += self.end_of_line
            report_entry = self.s4a_reporter_base[pin+8]
            report_entry = report_entry.replace("VALUE", value)
            responses += report_entry
            responses += self.end_of_line
    ...
    def s4a_OutPinOn(self, command):
        pin = int(command[1])
        
        self.firmata.s4a_OutPinOn(pin, 1)
        
        return 'okay'
        
    def s4a_OutPinOff(self, command):
        pin = int(command[1])
        
        self.firmata.s4a_OutPinOff(pin, 0)
        
        return 'okay'
        
    def s4a_analogOutPin(self, command):
        pin = int(command[1])
        data = int(command[2])
        
        self.firmata.s4a_analogOutPin(pin, data)
        
        return 'okay'
        
    def s4a_motoroff(self, command):
        pin = int(command[1])
        self.firmata.s4a_motoroff(pin)
        
        return 'okay'
        
    def s4a_MotorDirection(self, command):
        pin = int(command[1])
        
        if self.check_CMD_SERVO_DIR(command[2]) == 'clockwise':
            dir = 0
        elif self.check_CMD_SERVO_DIR(command[2]) == 'anticlockwise':
            dir = 180
            
        self.firmata.s4a_MotorDirection(pin, dir)
        
        return 'okay'
        
    def s4a_motorangle(self, command):
        pin = int(command[1])
        angle = int(command[2])
        self.firmata.s4a_MotorDirection(pin, angle)
    ...
    command_dict = {'crossdomain.xml': send_cross_domain_policy, 'reset_all': reset_arduino,
                    ...
                    'OutPinOn': s4a_OutPinOn, 'OutPinOff': s4a_OutPinOff, 'analogOutPin': s4a_analogOutPin, 
                    'motoroff': s4a_motoroff, 'MotorDirection': s4a_MotorDirection, 'motorangle': s4a_motorangle
                    }
