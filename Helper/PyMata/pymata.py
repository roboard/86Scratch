...
class PyMata:
    ...
    def __init__(self, port_id='/dev/ttyACM0', bluetooth=False, host_ip=None, host_port=None, verbose=True, ui_server_port=None, projectName=None, autodetect=0):
        """
        ...
        try:
            ...
            self.ui = PyMataServerSocket(('localhost', ui_server_port), self.command_deque_2)
            self.ui.open(self.verbose)
            self.ui.start()
            self._ui_command_handler = PyMataUICommandHandler(self)
            self._ui_command_handler.start()
            
            self.set_s2a_fm_status(0)
            self.set_error_no(2)
            ...
            if autodetect == 1:
                ...
                while tryConnect > 0:
                    s.settimeout(10)
                    m = s.recvfrom(256)
                    if ord(m[0][0]) == 0x86 and ord(m[0][1]) == 0x88 and ord(m[0][2]) == 0x88 and ord(m[0][3]) == 0x86:
                        if m[0].find(projectName) == 9:
                            host_ip = str(ord(m[0][4])) + '.' + str(ord(m[0][5])) + '.' + str(ord(m[0][6])) + '.' + str(ord(m[0][7]))
                            self._ui_command_handler.set_local_ip(ord(m[0][4]), ord(m[0][5]), ord(m[0][6]), ord(m[0][7]))
                            break
                        else:
            ...
            self.set_s2a_fm_status(1)

            if self.HOST and self.PORT:
                # Instantiate the socket support class
                self.transport = PyMataClientSocket((str(self.HOST), int(self.PORT)), self.command_deque, self._ui_command_handler)
                self._ui_command_handler.set_local_ip_str(str(self.HOST))
                self.operation_mode = 1
            else:
                # Instantiate the serial support class
                self.transport = PyMataSerial(port_id, self.command_deque, self._ui_command_handler)
            ...
            wait_client_timestart = time.time()
            if self.ui.clientStatus() == False:
                while time.time() - wait_client_timestart < 3:
                    if self.ui.clientStatus() == True:
                        break
            if self.ui.clientStatus() == True:
                self._ui_command_handler.send_error_message()
            self._ui_command_handler.stop()
            self.ui.stop()
            self.ui.close()
            raise
        except KeyboardInterrupt:
            self.set_error_no(99)
            wait_client_timestart = time.time()
            if self.ui.clientStatus() == False:
                while time.time() - wait_client_timestart < 3:
                    if self.ui.clientStatus() == True:
    ...
    def set_s2a_fm_status(self, ui_status):
        self._ui_command_handler.set_s2a_status(ui_status)

    def set_data_status(self, S2P, P2Duino, Duino2P, P2S):
        self._ui_command_handler.set_data_status(S2P, P2Duino, Duino2P, P2S)
    ...
    def get_encoder_response_table(self):
        ...
        return self._command_handler.get_encoder_response_table()
        
    def get_imu_response_table(self):
        
        return self._command_handler.get_imu_response_table()

    def get_imu_init_response_table(self):
        
        return self._command_handler.get_imu_init_response_table()

    def get_one_servo_response_table(self):
        
        return self._command_handler.get_one_servo_response_table()
    
    def get_s4a_response_table(self):
        
        return self._command_handler.get_s4a_response_table()
    ...
    def play_tone_educake(self, pin, tone_command, frequency, duration):
        """
        This method will call the Tone library for the selected pin.
        If the tone command is set to TONE_TONE, then the specified tone will be played.
        Else, if the tone command is TONE_NO_TONE, then any currently playing tone will be disabled.
    ...
    def set_encoder_mode(self, enable_disable, module, mode):
        command = [enable_disable, module, mode]
        
        self._command_handler.send_sysex(self._command_handler.ENCODER_CONFIG, command)
        
    def servo_config(self, pin, min_pulse=450, max_pulse=2800):
        """
    ...
    def move_one_servo_ex(self, pin, angle, mode, msec):
        """
        This method is added for 86Duino Servo library.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        data = [pin, angle & 0x7f, angle >> 7, mode, msec & 0x7F, msec >> 7]
        self._command_handler.send_sysex(self._command_handler.MOVE_ONE_SERVO, data)
        
    def config_servo_ex(self, pin, angle, mode, msec):
        """
        This method is added for 86Duino Servo library.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        data = [pin, angle & 0x7f, angle >> 7, mode, msec & 0x7F, msec >> 7]
        self._command_handler.send_sysex(self._command_handler.SERVO_CONFIG_EX, data)
    ...
    def test_data_query(self):
        ...
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        self._command_handler.send_sysex(self._command_handler.TEST_DATA_QUERY)
    ...
    def move_servo_all(self):
        ...
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        self._command_handler.send_sysex(self._command_handler.SERVO_MOVING_ALL)
    
    def init_imu(self, id):
        """
        This method is added dor 86Duino Servo library by Acen.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        data = [id & 0x7F, id >> 7]
        self._command_handler.send_sysex(self._command_handler.ENABLE_IMU, data)

    def s4a_OutPinOn(self, pin, on_off):
        """
        This method is added for 86Duino Servo library.
        """
        data = [pin, on_off & 0x7f, (on_off >> 7) & 0x7f]
        self._command_handler.send_sysex(self._command_handler.S4A_COMMAND, data)
    
    def s4a_OutPinOff(self, pin, on_off):
        """
        This method is added for 86Duino Servo library.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
    ...
    def s4a_analogOutPin(self, pin, value):
        """
        This method is added for 86Duino Servo library.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        data = [pin, value & 0x7f, (value >> 7) & 0x7f]
        self._command_handler.send_sysex(self._command_handler.S4A_COMMAND, data)
        
    def s4a_motoroff(self, pin):
        """
        This method is added for 86Duino Servo library.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
    ...
    def s4a_MotorDirection(self, pin, dir):
        """
        This method is added for 86Duino Servo library.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        data = [pin, dir & 0x7f, (dir >> 7) & 0x7f]
        self._command_handler.send_sysex(self._command_handler.S4A_COMMAND, data)
        
    def s4a_motorangle(self, pin, angle):
        """
        This method is added for 86Duino Servo library.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        data = [pin, angle & 0x7f, (angle >> 7) & 0x7f]
        self._command_handler.send_sysex(self._command_handler.S4A_COMMAND, data)
    ...
    def get_perform_motion_response_table(self):
        return self._command_handler.get_perform_motion_response_table()
    ...
    def set_error_no(self, error_no):
        self._ui_command_handler.set_error_no(error_no)
        
    def get_error_no(self):
        return self._ui_command_handler.get_error_no()
    
    def send_command_check_active(self, firmata):
        no_response_time = 0
        unstable_linking = 0
        while True:
            # operation_mode = 0 : USB Serial, Bluetooth mode, Enternet (high speed transmission)
            ...
            if self.operation_mode == 0:
                ...
                if self._command_handler._86Duino_active == 0:
                    no_response_time += 1
                    unstable_linking = 1
                    if no_response_time > 1:
                        self._ui_command_handler.set_error_no(7)
                        self._ui_command_handler.send_error_message()
                else:
                    if unstable_linking == 1:
                        unstable_linking = 0
                        self._ui_command_handler.set_error_no(100)
                        self._ui_command_handler.send_error_message()
                    self._command_handler._86Duino_active = 0
                    no_response_time = 0
